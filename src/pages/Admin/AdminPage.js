...
export const AdminPage = () => {
    ...
    const handleConfirmEdit = async () => {
        await DatabaseApi.adminEditUser(userToEdit.id, userToEdit.isPremium, userToEdit.isAdmin)
        reloadUserList();
        setShowEditModal(false);
        setUserToEdit(null);
    ...
    return (
        typeof storageAllUser !== "undefined" ? (
            <div className="admin-container">
                ...
                <ul>
                    ...
                    {storageAllUser.map((singleUser) => {
                        return (
                            <li key={singleUser?.id} className="list-title">
                                <h5>{singleUser?.id}</h5>
                                ...
                                <div className="admim-button">
                                    <button onClick={() => showModalOfEdit(singleUser)} className="edit-button">
                                    ...
                                    <button onClick={() => showModalOfExclude(singleUser)} className="delete-button">
                ...
                <Modal show={showModal} onHide={handleCloseModal} className="modal-main">
                    ...
                    <Modal.Footer>
                        <Button variant="secondary" className="modal-button-cancel" onClick={handleCloseModal}>
                            Cancelar
                        </Button>
                        <Button variant="danger" className="modal-button-save" onClick={() => handleConfirmDelete(userToDelete?.id)}>
                            Confirmar
                        </Button>
                    </Modal.Footer>
                </Modal>

                <Modal show={showEditModal} onHide={handleCloseEditModal} className="modal-main">
                    <Modal.Header closeButton>
                    ...
                    <Modal.Body className="modal-body">
                        <div>
                            <label>Id:</label>
                            <input type="text" value={userToEdit?.id || ''} onChange={handleEditNameChange} disabled />
                        </div>
                        <div>
                            <label>Name:</label>
                            <input type="text" value={userToEdit?.name || ''} onChange={handleEditNameChange} disabled />
                        </div> 
                        <div>
                            <label>isPremium:</label>
                            <select value={userToEdit?.isPremium || 0} onChange={handleEditIsPremiumChange}>
                        ...
                        <div>
                            <label>isAdmin:</label>
                            <select value={userToEdit?.isAdmin || 0} onChange={handleEditIsAdminChange}>
                                <option value={1}>true</option>
                                <option value={0}>false</option>
                            </select>
                        </div>
                    ...
                    <Modal.Footer>
                        <Button variant="secondary" className="modal-button-cancel" onClick={handleCloseEditModal}>
                            Cancelar
                        </Button>
                        <Button variant="primary" className="modal-button-save" onClick={handleConfirmEdit}>
                            Salvar
                        </Button>
                    </Modal.Footer>
                </Modal>
